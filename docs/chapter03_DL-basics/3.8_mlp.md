...
我们接下来通过`tensorflow.nn`提供的`relu`函数来绘制ReLU函数。可以看到，该激活函数是一个两段线性函数。
...
with tf.GradientTape() as g:
    g.watch(x)
    y = tf.nn.relu(x)
xyplot(x, g.gradient(y, x), 'grad of relu')
```
<div align=center>
...
with tf.GradientTape() as g:
    g.watch(x)
    y = tf.nn.sigmoid(x)
xyplot(x, g.gradient(y, x), 'grad of sigmoid')
```
<div align=center>
...
with tf.GradientTape() as g:
    g.watch(x)
    y = tf.nn.tanh(x)
xyplot(x, g.gradient(y, x), 'grad of tanh')
```
<div align=center>
